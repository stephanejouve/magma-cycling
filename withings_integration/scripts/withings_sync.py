...
INTERVALS_API_KEY = os.getenv("INTERVALS_API_KEY", "420dlwmr1rxqfb73z19iq0ime")
...
def sync_daily_data(days_back: int = 1):
    ...
    """
    print("\n" + "=" * 70)
    print("SYNCHRONISATION WITHINGS → INTERVALS.ICU")
    ...
    try:
        ...
        if weight_data_list:
            ...
            for weight_data in weight_data_list:
                ...
                try:
                    sync_weight_to_intervals(weight_data, INTERVALS_API_KEY, INTERVALS_ATHLETE_ID)
                except Exception as e:
    ...
    try:
        sleep_data_list = withings.get_sleep_data(start_date.date(), end_date.date())
        ...
        if sleep_data_list:
            ...
            for sleep_data in sleep_data_list:
                ...
                # Synchroniser vers Intervals.icu
                try:
                    sync_sleep_to_intervals(sleep_data, INTERVALS_API_KEY, INTERVALS_ATHLETE_ID)
                except Exception as e:
                    print(f"    ⚠ Erreur sync: {e}")
...
def display_weekly_summary():
    ...
    if sleep_data:
        ...
        nights_above_7h = sum(
            1 for s in sleep_data if s["total_sleep_hours"] and s["total_sleep_hours"] >= 7
        )

        print("😴 SOMMEIL (7 derniers jours)")
        ...
        if sleep_debt > 5:
            print("   ⚠️  DETTE IMPORTANTE - Prioriser récupération")
        elif sleep_debt > 0:
            print("   ⚠️  Légère dette - Attention charge entraînement")
        else:
            print("   ✅ Sommeil optimal")
