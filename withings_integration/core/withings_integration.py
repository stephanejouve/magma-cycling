    ...
    def get_sleep_data(self, start_date: datetime, end_date: datetime) -> List[Dict]:
        ...
        for series in sleep_summary.series:
            sleep_entry = {
                ...
                'total_sleep_hours': round(series.data.total_sleep_time / 3600, 2) if series.data.total_sleep_time else None,
                'deep_sleep_minutes': round(series.data.deepsleepduration / 60, 1) if series.data.deepsleepduration else None,
                'light_sleep_minutes': round(series.data.lightsleepduration / 60, 1) if series.data.lightsleepduration else None,
                'rem_sleep_minutes': round(series.data.remsleepduration / 60, 1) if series.data.remsleepduration else None,
                'wakeup_count': series.data.wakeupcount if series.data.wakeupcount else 0,
                'wakeup_minutes': round(series.data.wakeupduration / 60, 1) if series.data.wakeupduration else None,
                'sleep_score': series.data.sleep_score if series.data.sleep_score else None,
                'breathing_disturbances': series.data.breathing_disturbances_intensity if series.data.breathing_disturbances_intensity else None
            }
...
def sync_weight_to_intervals(withings_data: Dict, intervals_api_key: str, athlete_id: str):
    ...
    headers = {
        'Authorization': f'Basic {intervals_api_key}',
        'Content-Type': 'application/json'
    }
    
    data = {
        'weight': withings_data['weight_kg']
    }
...
def sync_sleep_to_intervals(sleep_data: Dict, intervals_api_key: str, athlete_id: str):
    ...
    sleep_seconds = int(sleep_data['total_sleep_hours'] * 3600) if sleep_data['total_sleep_hours'] else None
...
if __name__ == "__main__":
    ...
    if sleep:
        ...
        print(f"\n{'='*50}")
        print(f"RECOMMANDATIONS ENTRAÎNEMENT:")
        print(f"{'='*50}")
