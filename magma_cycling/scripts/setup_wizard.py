...
def _monday_of_current_week(today: date | None = None) -> date:
    """Renvoie le lundi de la semaine courante (ou de ``today`` si fourni)."""
    today = today or date.today()
    return today - timedelta(days=today.weekday())


def _create_week_reference_config(
    data_repo_path: Path,
    athlete_id: str,
    name: str,
    s001_date: date | None = None,
) -> bool:
    """Cree ``.config.json`` multi-season si absent.

    Structure minimale compatible avec ``WeekReferenceConfig`` : une seule
    saison ``initial`` demarrant le lundi de la semaine courante. L'utilisateur
    peut completer manuellement (saisons supplementaires, global_week_end,
    active_season differente) apres le setup.

    Idempotent : si le fichier existe, ne l'ecrase pas.

    Returns True si le fichier a ete cree, False s'il existait deja.
    """
    config_path = data_repo_path / ".config.json"
    if config_path.exists():
        return False

    s001 = s001_date or _monday_of_current_week()
    payload = {
        ...
        "week_reference": {
            "seasons": {
                "initial": {
                    "s001_date": s001.isoformat(),
                    "description": (
                        f"Initial season (auto-generated by setup_wizard on "
...
def _init_data_repo(data_repo_path: Path, athlete_id: str = "", name: str = ""):
    """Initialise l'espace de donnees (git init, fichiers, arborescence).

    Si ``athlete_id`` et ``name`` sont fournis, genere aussi ``.config.json``
    multi-season (saison ``initial`` demarrant le lundi courant). Idempotent :
    ne reecrit pas ``.config.json`` s'il existe deja.

    Git operations are skipped silently if git is not installed.
    """
    already_initialized = (
        data_repo_path.exists() and (data_repo_path / "workouts-history.md").exists()
    )

    if already_initialized:
        print_success(f"Deja en place : {data_repo_path}")
        _ensure_data_directories(data_repo_path)
        # Tente quand meme la creation du .config.json (cas install pre-existante
        # sans config). No-op si deja present.
        if athlete_id and name and _create_week_reference_config(data_repo_path, athlete_id, name):
            print_success(f"Config semaine cree : {data_repo_path / '.config.json'}")
        return
    ...
    # Config semaine multi-season (requise par WeekReferenceConfig pour
    # toutes les features week_reference-dependent : daily-sync, end-of-week,
    # backfill, etc.)
    if athlete_id and name:
        _create_week_reference_config(data_repo_path, athlete_id, name)
...
class SetupWizard:
    ...
    def step_data_repo(self):
        ...
        _init_data_repo(
            ...
            name=self.config.get("name") or self.config.get("api_name", ""),
        )
