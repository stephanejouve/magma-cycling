...
def select_file(files: list[Path]) -> Path:
    ...
    while True:
        try:
            choice = input(f"\n❓ Choisir le fichier à nettoyer (1-{len(files)}) ou 'all' pour tous: ").strip().lower()
...
def clean_file(file_path: Path, auto_confirm: bool = False) -> bool:
    ...
    for dup in duplicates:
        print(f"\n  {dup['id']}:")
        print(f"    Première occurrence: lignes {dup['first']['start_line']+1}-{dup['first']['end_line']+1}")
        print(f"    Doublon à supprimer: lignes {dup['duplicate']['start_line']+1}-{dup['duplicate']['end_line']+1}")
    ...
    # Créer backup
    backup_path = file_path.parent / f"workouts-history.BACKUP_{datetime.now().strftime('%Y%m%d_%H%M%S')}.md"
