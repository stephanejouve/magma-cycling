...
Usage:
    python3 scripts/collect_athlete_feedback.py
    python3 scripts/collect_athlete_feedback.py --quick  # Mode rapide (RPE + ressenti uniquement)
    python3 scripts/collect_athlete_feedback.py --clear  # Effacer le feedback en attente
"""
...
class AthleteFeedbackCollector:
    ...
    def collect_full_feedback(self):
        ...
        if self.prompt_yes_no("❓ Difficultés rencontrées durant la séance ?", default=False):
            feedback['difficultes'] = self.prompt_multiline(
                "📝 Décrivez les difficultés"
            )
        else:
        ...
        if self.prompt_yes_no("✨ Points positifs à noter ?", default=True):
            feedback['points_positifs'] = self.prompt_multiline(
                "📝 Décrivez les points positifs"
            )
        else:
...
def main():
    parser = argparse.ArgumentParser(
        description="Collecter le retour athlète après séance"
    )

    parser.add_argument(
        '--quick',
        action='store_true',
        help="Mode rapide (RPE + ressenti uniquement)"
    )
    parser.add_argument(
        '--clear',
        action='store_true',
        help="Effacer le feedback en attente"
    )
    parser.add_argument(
        '--show',
        action='store_true',
        help="Afficher le feedback en attente"
    )
    parser.add_argument(
        '--temp-dir',
    ...
    parser.add_argument(
        '--activity-name',
        help="Nom de la séance en cours d'analyse (mode batch)"
    )
    parser.add_argument(
        '--activity-date',
        help="Date de la séance (ex: 2025-11-21)"
    )
    parser.add_argument(
        '--activity-duration',
        type=int,
        help="Durée de la séance en minutes"
    )
    parser.add_argument(
        '--activity-tss',
        type=float,
        help="TSS de la séance"
    )
    parser.add_argument(
        '--activity-if',
        type=float,
        help="IF (Intensity Factor) de la séance"
    )
    parser.add_argument(
        '--batch-position',
        help="Position dans le batch (format: 1/3, 2/3, etc.)"
    )
    ...
    try:
        ...
        if collector.prompt_yes_no("\n💾 Sauvegarder ce feedback ?", default=True):
            ...
            print()
            print("   ./scripts/prepare_analysis.py")
        else:
