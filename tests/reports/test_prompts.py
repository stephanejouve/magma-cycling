...
class TestBuildBilanFinalPrompt:
    """Tests for build_bilan_final_prompt function (Day 3, updated Day 5)."""

    def test_build_prompt_with_valid_data(self):
        """Test prompt building with valid bilan_final data."""
        # Given: Valid week data (same structure as workout_history)
        week_data = SAMPLE_WEEK_DATA_S076
    ...
    def test_prompt_contains_week_context(self):
        """Test prompt includes week context and data."""
        # Given: Week data with full context
        week_data = SAMPLE_WEEK_DATA_S076

        # When: Building prompt
        prompt = build_bilan_final_prompt(week_data)

        # Then: Prompt should include dates, TSS, activities
        assert "2026-01-13" in prompt
        assert "2026-01-19" in prompt
        assert "450" in prompt  # TSS planned
        assert "423" in prompt  # TSS realized
