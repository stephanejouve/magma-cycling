...
def generate_markdown_examples(skipped_sessions):
    ...
    markdown = checker.generate_skipped_session_markdown(
        skipped_session=session, metrics_pre={"ctl": 55, "atl": 48, "tsb": 7}
    )
...
def main():
    ...
    parser.add_argument(
        "--generate-markdown", action="store_true", help="Générer exemples de markdown"
    )
    parser.add_argument("--verbose", action="store_true", help="Mode verbose (debug)")
